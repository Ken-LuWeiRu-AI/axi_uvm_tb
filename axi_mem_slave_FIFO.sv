...
module axi_mem_slave #(
  ...
  always_ff @(posedge ACLK or negedge ARESETn) begin
    if (!ARESETn) begin
      ...
      // -------------------------
      // Variable Declarations (moved to top of block)
      // -------------------------
      bit aw_push, aw_pop;
      bit ar_push, ar_pop;
      ...
      aw_pop  = (w_state == W_IDLE) && !awq_empty();
      ...
      if (aw_push && !aw_pop)      awq_count <= awq_count + 1;
      else if (!aw_push && aw_pop) awq_count <= awq_count - 1;
      // else if (aw_push && aw_pop) count stays the same
      ...
      case (w_state)
        ...
        W_RESP_WAIT: begin
          if (w_resp_cnt + 1 >= WR_RESP_LATENCY) w_state <= W_RESP;
          else w_resp_cnt <= w_resp_cnt + 1;
        end
      ...
      ar_pop  = (r_state == R_IDLE) && !arq_empty();
      ...
      if (ar_push && !ar_pop)      arq_count <= arq_count + 1;
      else if (!ar_push && ar_pop) arq_count <= arq_count - 1;
      // else if (ar_push && ar_pop) count stays the same
      ...
      case (r_state)
        ...
        R_SEND: begin
          // --- FIXED: Declarations MUST be at the top of the begin block ---
          bit beat_hs;
          ...
          // 2. Drive Data (Start or Next)
          else if (!RVALID || beat_hs) begin
             // If RVALID=0, we are starting beat 'r_beat_idx' (idx=0).
             // If RVALID=1 (beat_hs), we are moving to 'r_beat_idx + 1'.
             next_idx = (RVALID) ? (r_beat_idx + 1) : r_beat_idx;

             beat_addr   = araddr_q + (next_idx * r_bytes);
             ...
             // Advance counter only if the previous valid beat completed
